...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'publish') {
        await assertOwnership(pool, id, ctx);
        if (!ctx.isAdmin) return res.json(await handleReviewSubmit(pool, id, ctx));
      ...
      if (action === 'submit-for-review') { await assertOwnership(pool, id, ctx); return res.json(await handleReviewSubmit(pool, id, ctx)); }
      if (action === 'approve-review') { adminOnly(); return res.json(await handleReviewApprove(pool, id, body, ctx)); }
      if (action === 'reject-review')  { adminOnly(); return res.json(await handleReviewReject(pool, id, body, ctx)); }
...
// ── Review queue (admin) ───────────────────────────────────────────────────────
async function handleReviewQueue(pool, statusFilter) {
...
// ── Review detail (admin) ──────────────────────────────────────────────────────
async function handleReviewDetail(pool, id) {
  if (!id) throw Object.assign(new Error('id is required'), { status: 400 });
  const { rows } = await pool.query(`
    SELECT
      i.*,
...
async function handleReviewSubmit(pool, id, ctx) {
  if (!id) throw Object.assign(new Error('id is required'), { status: 400 });
...
async function handleReviewApprove(pool, id, body, ctx) {
  if (!id) throw Object.assign(new Error('id is required'), { status: 400 });
  ...
  const { rows } = await pool.query(`
    UPDATE "Itinerary" SET
      status              = 'published',
      "isPublished"       = true,
      "isPrivate"         = CASE WHEN type != 'custom' THEN false ELSE "isPrivate" END,
      "reviewedAt"        = NOW(),
      "reviewedByUserId"  = $2,
      "reviewMessage"     = $3,
  ...
  await pool.query(
    ...
    [id]
  ).catch(err => console.warn('[itinerary-cms/approve-review] CustomRequest sync failed:', err.message));

  return { itinerary: rows[0], message: 'Itinerary approved and published.' };
}

// ── Reject (admin only) ───────────────────────────────────────────────────────
async function handleReviewReject(pool, id, body, ctx) {
  if (!id) throw Object.assign(new Error('id is required'), { status: 400 });
